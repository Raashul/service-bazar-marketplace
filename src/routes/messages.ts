...
router.post("/", async (req: Request, res: Response) => {
  try {
    const { product_id, buyer_id, message }: CreateMessageRequest = req.body;
    ...
    const existingMessage = await pool.query(
      "SELECT id FROM messages WHERE product_id = $1 AND buyer_id = $2 AND seller_id = $3",
...
router.put("/respond", async (req: Request, res: Response) => {
  try {
    const { message_id, seller_id, status }: MessageResponseRequest = req.body;
