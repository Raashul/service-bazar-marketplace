    ...
    try {
        ...
        if (!title ||
            ...
            return res.status(400).json({
                error: "Required fields: title, description, price, category, condition",
            });
        }
...
router.get("/", async (req, res) => {
    try {
        const { category, subcategory, subsubcategory, min_price, max_price, condition, search, status = "active", page = 1, limit = 20, } = req.query;
        let query = `
      SELECT p.*, u.name as seller_name, u.phone as seller_phone 
      FROM products p 
      JOIN users u ON p.seller_id = u.id 
...
router.put("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, price, currency, category, subcategory, subsubcategory, condition, is_negotiable, expires_in_days, } = req.body;
